...
namespace ZenFulcrum.EmbeddedBrowser
{
    ...
    {
        private static VRCursor _instance;
        public static VRCursor Instance
        {
            get
            {
                if (!_instance)
                {
                    _instance = FindObjectOfType<VRCursor>();
                    if (!_instance)
                    {
                        var go = new GameObject("Cursor Crosshair");
                        _instance = go.AddComponent<VRCursor>();
                    }
                }
                return _instance;
            }
        ...
        public static void SetUpBrowserInput(Browser browser, MeshCollider mesh)
        {
            ...
            var fpsUI = VRBrowserUI.Create(mesh, pointer, crossHair);
